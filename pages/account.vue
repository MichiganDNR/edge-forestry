<template>
  <Appear>
    ...
    <div class="max-w-xl mx-auto mt-10 bg-white p-6 rounded-xl shadow">
      ...
      <div v-if="checkingAuth" class="text-center text-gray-500">Checking authentication...</div>
      <div v-else-if="loading" class="text-center text-gray-500">Loading...</div>
      <div v-else-if="error" class="text-center text-red-500">{{ error }}</div>
      <div v-else class="space-y-4 text-lg text-green-900">
        <div>
...
const handleLogout = async () => {
  try {
    await signOut($auth)
    userStore.logout()       // clear Pinia store
    router.push('/')    
  } catch (err) {
